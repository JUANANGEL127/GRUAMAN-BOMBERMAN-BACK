...
async function handleResumen(req, res) {
  try {
    ...
    let total_horas_trabajadas = 0;
    let total_extra_diurna = 0;
    let total_extra_nocturna = 0;
    let total_extra_festiva = 0;
    const registros = [];

    for (const r of q.rows) {
      const fecha = r.fecha_servicio && r.fecha_servicio.toISOString ? r.fecha_servicio.toISOString().slice(0,10) : formatDateOnly(r.fecha_servicio);
      if (r.hora_ingreso && r.hora_salida && (r.minutos_almuerzo !== undefined)) {
        const calculos = calcularHoras({ hora_ingreso: r.hora_ingreso, hora_salida: r.hora_salida, minutos_almuerzo: r.minutos_almuerzo, fecha });
        total_horas_trabajadas += calculos.horas_trabajadas || 0;
        total_extra_diurna += calculos.extra_diurna || 0;
        total_extra_nocturna += calculos.extra_nocturna || 0;
        total_extra_festiva += calculos.extra_festiva || 0;
        registros.push({ ...r, fecha_servicio: fecha, ...calculos });
      }
    }
    ...
    return res.json({
      ...
      resumen: {
        total_horas_trabajadas: +total_horas_trabajadas.toFixed(2),
        total_extra_diurna: +total_extra_diurna.toFixed(2),
        total_extra_nocturna: +total_extra_nocturna.toFixed(2),
        total_extra_festiva: +total_extra_festiva.toFixed(2),
        total_horas: +((total_horas_trabajadas + total_extra_diurna + total_extra_nocturna + total_extra_festiva)).toFixed(2)
      },
      registros
    });
  } catch (err) {
...
async function handleDescargar(req, res) {
  try {
    ...
    if (formato === 'excel') {
      ...
      const ws = workbook.addWorksheet('Horas Jornada');
      if (rows.length > 0) {
