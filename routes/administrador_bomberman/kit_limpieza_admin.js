...
router.post('/buscar', async (req, res) => {
  try {
    ...
    console.error('Error en kit_limpieza_admin/buscar:', err);
...
async function generarPDF(r) {
  try {
    const candidatePaths = [
      path.join(process.cwd(), 'templates', 'kit_limpieza_template.xlsx'),
      path.join(process.cwd(), 'routes', 'templates', 'kit_limpieza_template.xlsx'),
      path.join(process.cwd(), 'routes', 'administrador_bomberman', 'templates', 'kit_limpieza_template.xlsx')
    ];
    const tplPath = candidatePaths.find(p => fs.existsSync(p));
    if (!tplPath) throw new Error('Template XLSX no encontrado. Agrega kit_limpieza_template.xlsx en la carpeta templates.');
    ...
    console.error('Error en generarPDF kit_limpieza:', err);
    throw err;
  }
}
...
router.post('/descargar', async (req, res) => {
  try {
    ...
    if (formato === 'excel') {
      const workbook = new ExcelJS.Workbook();
      const ws = workbook.addWorksheet('Kit Lavado Mantenimiento');
      if (!q.rows.length) {
        res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition','attachment; filename=kit_limpieza.xlsx');
        await workbook.xlsx.write(res);
        return res.end();
      }
      const keys = Object.keys(q.rows[0]);
      ws.addTable({
        name: 'TablaKitLimpieza',
        ref: 'A1',
        headerRow: true,
        totalsRow: false,
    ...
    if (formato === 'pdf') {
      ...
      res.setHeader('Content-Disposition','attachment; filename="kit_limpieza.zip"');
      ...
      for (const r of q.rows) {
        try {
          ...
          archive.append(buf, { name: `kit_limpieza_${r.id}.pdf` });
        } catch(e) {
          archive.append(`Error id=${r.id}: ${e.message}`, { name: `kit_limpieza_${r.id}_error.txt` });
        }
      }
    ...
    console.error('Error en kit_limpieza_admin/descargar:', err);
    res.status(500).json({ success:false, error: err.message });
  }
});
