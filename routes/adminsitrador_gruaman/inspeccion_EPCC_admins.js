...
router.get('/inspeccion_epcc/search', async (req, res) => {
  try {
    ...
    const allowed = ['nombre_cliente','nombre_proyecto','fecha','fecha_from','fecha_to','nombre_operador','cargo','serial_arnes'];
    const { where, values } = buildWhere(req.query, allowed);
    const limit = Math.min(200, parseInt(req.query.limit) || 100);
    const offset = parseInt(req.query.offset) || 0;
    const finalQuery = `SELECT * FROM inspeccion_epcc ${where} ORDER BY id DESC LIMIT $${values.length+1} OFFSET $${values.length+2}`;
    const q = await pool.query(finalQuery, [...values, limit, offset]);
    res.json({ success: true, count: q.rowCount, rows: q.rows });
  } catch (err) {
    console.error('Error searching inspeccion_epcc:', err);
...
router.post('/buscar', async (req, res) => {
  try {
    ...
    const q = await pool.query(
      `SELECT * FROM inspeccion_epcc ${where} ORDER BY id DESC LIMIT $${idx} OFFSET $${idx+1}`,
    ...
    console.error('Error en /inspeccion_EPCC_admins/buscar:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
router.post('/descargar', async (req, res) => {
  try {
    ...
    const where = clauses.length ? 'WHERE ' + clauses.join(' AND ') : '';
    const q = await pool.query(`SELECT * FROM inspeccion_epcc ${where} ORDER BY id DESC LIMIT $${idx}`, [...values, Math.min(50000, parseInt(limit) || 10000)]);
    ...
    console.error('Error en /inspeccion_EPCC_admins/descargar:', err);
    res.status(500).json({ success:false, error: err.message });
  }
});
