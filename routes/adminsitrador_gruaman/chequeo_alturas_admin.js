...
/**
 * GET /adminsitrador_gruaman/chequeo_alturas/search
 * Búsqueda flexible por query string en registros de inspección de trabajo en alturas.
 ...
 */
router.get('/chequeo_alturas/search', async (req, res) => {
  try {
    ...
    const finalQuery = `SELECT * FROM chequeo_alturas ${where} ORDER BY id DESC LIMIT $${values.length+1} OFFSET $${values.length+2}`;
    const q = await pool.query(finalQuery, [...values, limit, offset]);
    res.json({ success: true, count: q.rowCount, rows: q.rows });
  } catch (err) {
    console.error('Error searching chequeo_alturas:', err);
 ...
 * Busca registros de inspección de trabajo en alturas usando filtros del body.
...
router.post('/buscar', async (req, res) => {
  try {
    ...
    const q = await pool.query(
      `SELECT * FROM chequeo_alturas ${where} ORDER BY id DESC LIMIT $${idx} OFFSET $${idx+1}`,
    ...
    console.error('Error en /chequeo_alturas_admin/buscar:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

/**
 * Genera un PDF de registro único para una inspección de trabajo en alturas.
 * @param {object} r - Fila de BD de chequeo_alturas.
 * @returns {Promise<Buffer>}
 */
async function generarPDFPorChequeoAlturas(r) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(16).text(`Chequeo Alturas - ID: ${r.id}`, { align: 'center' });
      doc.moveDown();
      ...
      doc.moveDown();
      const campos = [
        'sintomas_fisicos','medicamento','consumo_sustancias','condiciones_fisicas_mentales','lugar_trabajo_demarcado',
        'inspeccion_medios_comunicacion','equipo_demarcado_seguro','base_libre_empozamiento','iluminacion_trabajos_nocturnos',
        'uso_adecuado_epp_epcc','uso_epp_trabajadores','epcc_adecuado_riesgo','interferencia_otros_trabajos',
        'observacion_continua_trabajadores','punto_anclaje_definido','inspeccion_previa_sistema_acceso',
        ...
        'informacion_riesgos_trabajadores','distancias_minimas_seguridad','tablero_libre_elementos_riesgo',
        'cables_en_buen_estado','observaciones'
      ];
      campos.forEach(k => {
 ...
 * Exporta registros filtrados de inspección de trabajo en alturas en el formato solicitado.
...
router.post('/descargar', async (req, res) => {
  try {
    ...
    const where = clauses.length ? 'WHERE ' + clauses.join(' AND ') : '';
    const q = await pool.query(`SELECT * FROM chequeo_alturas ${where} ORDER BY id DESC LIMIT $${idx}`, [...values, Math.min(50000, parseInt(limit) || 10000)]);

    if (formato === 'excel') {
      const workbook = new ExcelJS.Workbook();
      const ws = workbook.addWorksheet('Chequeo Alturas');

      if (!q.rows || q.rows.length === 0) {
        res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition','attachment; filename=chequeo_alturas.xlsx');
    ...
    if (formato === 'pdf') {
      ...
      res.setHeader('Content-Disposition','attachment; filename="chequeo_alturas.zip"');
      const archive = archiver('zip', { zlib: { level: 9 } });
      archive.on('error', err => { console.error('Archiver error:', err); try{ res.status(500).end(); } catch(e){} });
      archive.pipe(res);
      for (const r of q.rows) {
        try {
          const pdfBuf = await generarPDFPorChequeoAlturas(r);
          archive.append(pdfBuf, { name: `chequeo_alturas_${r.id}.pdf` });
        } catch (pdfErr) {
          archive.append(`Error generando PDF para id=${r.id}: ${pdfErr.message||pdfErr}`, { name: `chequeo_alturas_${r.id}_error.txt` });
        }
      }
    ...
    res.setHeader('Content-Disposition','attachment; filename="chequeo_alturas.csv"');
    return res.send(csv);

  } catch (err) {
    console.error('Error en /chequeo_alturas_admin/descargar:', err);
    res.status(500).json({ success:false, error: err.message });
  }
});
