...
/**
 * GET /adminsitrador_gruaman/chequeo_torregruas/search
 * Búsqueda flexible por query string en registros de inspección diaria de torre grúa.
 ...
 */
router.get('/chequeo_torregruas/search', async (req, res) => {
  try {
    ...
    const finalQuery = `SELECT * FROM chequeo_torregruas ${where} ORDER BY id DESC LIMIT $${values.length+1} OFFSET $${values.length+2}`;
    const q = await pool.query(finalQuery, [...values, limit, offset]);
    res.json({ success: true, count: q.rowCount, rows: q.rows });
  } catch (err) {
    console.error('Error searching chequeo_torregruas:', err);
 ...
 * Busca registros de inspección de torre grúa usando filtros del body.
...
router.post('/buscar', async (req, res) => {
  try {
    ...
    const q = await pool.query(
      `SELECT * FROM chequeo_torregruas ${where} ORDER BY id DESC LIMIT $${idx} OFFSET $${idx+1}`,
    ...
    console.error('Error en /chequeo_torregruas_admin/buscar:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

/**
 * Genera un PDF de registro único para una inspección de torre grúa.
 * @param {object} r - Fila de BD de chequeo_torregruas.
 * @returns {Promise<Buffer>}
 */
async function generarPDFPorChequeo(r) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(16).text(`Chequeo Torre Grúa - ID: ${r.id}`, { align: 'center' });
      doc.moveDown();
      ...
      doc.moveDown();
      const campos = [
        'epp_personal','epp_contra_caidas','ropa_dotacion','tornilleria_ajustada','anillo_arriostrador',
        'soldaduras_buen_estado','base_buenas_condiciones','funcionamiento_pito','cables_alimentacion',
        'movimientos_maquina','cables_enrollamiento','frenos_funcionando','poleas_dinamometrica',
        'gancho_seguro','punto_muerto','mando_buen_estado','baldes_buen_estado','canasta_materiales',
        'estrobos_buen_estado','grilletes_buen_estado','ayudante_amarre','radio_comunicacion','observaciones'
      ];
      campos.forEach(k => {
 ...
 * Exporta registros filtrados de inspección de torre grúa en el formato solicitado.
...
router.post('/descargar', async (req, res) => {
  try {
    ...
    const where = clauses.length ? 'WHERE ' + clauses.join(' AND ') : '';
    const q = await pool.query(`SELECT * FROM chequeo_torregruas ${where} ORDER BY id DESC LIMIT $${idx}`, [...values, Math.min(50000, parseInt(limit) || 10000)]);

    if (formato === 'excel') {
      const workbook = new ExcelJS.Workbook();
      const ws = workbook.addWorksheet('Chequeo Torre Grúa');

      if (!q.rows || q.rows.length === 0) {
        res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition','attachment; filename=chequeo_torregruas.xlsx');
    ...
    if (formato === 'pdf') {
      ...
      res.setHeader('Content-Disposition','attachment; filename="chequeo_torregruas.zip"');
      const archive = archiver('zip', { zlib: { level: 9 } });
      archive.on('error', err => { console.error('Archiver error:', err); try{ res.status(500).end(); } catch(e){} });
      archive.pipe(res);
      for (const r of q.rows) {
        try {
          const pdfBuf = await generarPDFPorChequeo(r);
          archive.append(pdfBuf, { name: `chequeo_torregruas_${r.id}.pdf` });
        } catch (pdfErr) {
          archive.append(`Error generando PDF para id=${r.id}: ${pdfErr.message||pdfErr}`, { name: `chequeo_torregruas_${r.id}_error.txt` });
        }
      }
    ...
    res.setHeader('Content-Disposition','attachment; filename="chequeo_torregruas.csv"');
    return res.send(csv);

  } catch (err) {
    console.error('Error en /chequeo_torregruas_admin/descargar:', err);
    res.status(500).json({ success:false, error: err.message });
  }
});
